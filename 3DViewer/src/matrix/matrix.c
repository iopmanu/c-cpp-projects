...
int s21_eq_matrix(matrix_t *A, matrix_t *B) {
    ...
    for (int i = 0; (i < A->rows) && (comparison_result == SUCCESS); i++) {
        for (int j = 0; (j < B->columns) && (comparison_result == SUCCESS); j++) {
            if (fabs(A->matrix[i][j] - B->matrix[i][j]) > 1e-7) comparison_result = FAILURE;
        }
    }
...
int s21_sum_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
    ...
    CHECK((A->rows < 1) || (A->columns < 1), S21_INVALID_MATRIX);
    CHECK((B->rows < 1) || (B->columns < 1), S21_INVALID_MATRIX);
    CHECK((A->rows != B->rows) || (A->columns != B->columns), S21_MATH_OPERATIONS_ERROR);
...
double s21_determinant_recursive(matrix_t *A) {
    ...
    if (A->rows == 1) {
        ...
        for (int i = 0; i < A->rows; i++) {
            ...
            determinant += sign * A->matrix[0][i] * s21_determinant_recursive(&minor);
            sign = -sign;
...
enum S21_MATRIX_CODE s21_get_minor(int row, int column, matrix_t *A, matrix_t *result) {
    ...
    enum S21_MATRIX_CODE code = s21_create_matrix(A->rows - 1, A->columns - 1, result);
    CHECK(code, S21_INVALID_MATRIX);
